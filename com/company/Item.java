...
public class Item {
    ...
    public void returnTypeOfWeapon(){
        if (this.type == 1 || this.type == 10){
            System.out.println("weapon");
        }
        else if (this.type == 2 || this.type == 20){
    ...
    private int weaponEffect(int cAtt,int mDef,int cLuck){
        ...
        switch (this.effectID){
            case 6:
                tempCal = cAtt + (int) Math.random()*50;
                tempCal = checkDmgCalculation(tempCal,mDef);
                break;
        }
        return tempCal;
    }
    ...
    public int[] changeCharValues(){
        ...
        int[] items = new int[]{charHp,charMp, charLuck,  charSpeed};
        return items;
    }
}